...
We include the initial values of the scalar field, $\phi_0$, and its time derivative, $\dot{\phi}_0$ in the list of free parameters; the quintessence model does not nessecarily have attractor solutions, and so varying the initial conditions should be expected to vary the final outcome. The fiducial values of these parameters are given in \tref{tab:fid_params}. Using a simple algorithm we explain below, we search the parameter space for the combination of model and cosmological parameters for whose solutions the sequestering constraint (\ref{eq:vanish_R}) holds. 
 ...
 \begin{figure*}[!t]
	...
	\subfigure[\, Maximum size of the Universe, $\qsubrm{a}{max} $]{\includegraphics[scale=0.5]{images/wl/test22_2_amax}}\qquad
...
\bea
\rbm{p} = \bigg\{\qsubrm{\Omega}{m}h^2,\qsubrm{\Omega}{b}h^2, \dot{\phi}_0,m^3 \bigg\}
\eea

 \begin{figure}[!t]
      \begin{center}
      	\subfigure[\, Likelihood ]{\includegraphics[scale=0.3]{images/seqs_1d/test22_33_L}} \qquad
	\subfigure[\, Derived spatial curvature ]{\includegraphics[scale=0.3]{images/seqs_1d/test22_33_Omk}}\\
	\subfigure[\, $\qsubrm{a}{max}$ ]{\includegraphics[scale=0.3]{images/seqs_1d/test22_33_amax}} \qquad
	\subfigure[\, $\qsubrm{t}{max}$ ]{\includegraphics[scale=0.3]{images/seqs_1d/test22_33_tmax}} 
      \end{center}
